...
def logAnalyze(fileType,line ,analyze_datetime):
    jobtype=('過去ZIP移動','ZIP取り込み','ZIPファイル展開','ファイル変換','可読化ファイル結合','可読化ファイル連携','マスタ更新','マスタ同期')
    OutputList=[]
    ###TLDP007I001  ZIP 過去フォルダから実施フォルダに移動
    list_TLDP007I001= list(filter(lambda x: x[0]=='TLDP007I001', line))
    if len(list_TLDP007I001) >0:
        tLDP007_FirstAppearIndex = line.index(list_TLDP007I001[0])
        # TLDP007I001がはじめ出た行の以後から、TLDP007I002を収集
        list_TLDP007I002 = list(filter(lambda x: x[0]=='TLDP001I002', line[tLDP007_FirstAppearIndex:]))
        for tLDP001, tLDP002 in zip(list_TLDP007I001, list_TLDP007I002):
            t_time = timeCaculation(tLDP001,tLDP002)
            OutputList.append('{0},{1},{2},{3}'.format('過去ZIP移動',str(tLDP001[1]),str(tLDP002[1]),str(t_time)))
    ###TLDP007I001  ZIP 過去フォルダから実施フォルダに移動
    ...
    ###TLDP006I001 マスタ更新
    list_TLDP006I001= list(filter(lambda x: x[0]=='TLDP006I001', line))
    if len(list_TLDP006I001) > 0:
        tLDP006_FirstAppearIndex = line.index(list_TLDP006I001[0])
        # TLDP006I001がはじめ出た行の以後から、TLDP006I002を収集
        list_TLDP006I002 = list(filter(lambda x:  x[0]=='TLDP006I002', line[tLDP006_FirstAppearIndex:]))
        for tLDP001, tLDP002 in zip(list_TLDP006I001, list_TLDP006I002):
            t_time = timeCaculation(tLDP001,tLDP002)
            OutputList.append('{0},{1},{2},{3}'.format('マスタ更新', str(tLDP001[1]), str(tLDP002[1]), str(t_time)))

    ###TLDP006I001 マスタ更新


    ###TLDP008I001 マスタ同期
    list_TLDP008I001= list(filter(lambda x: x[0]=='TLDP006I001', line))
    if len(list_TLDP008I001) > 0:
        tLDP008_FirstAppearIndex = line.index(list_TLDP008I001[0])
        # TLDP008I001がはじめ出た行の以後から、TLDP008I002を収集
        list_TLDP008I002 = list(filter(lambda x:  x[0]=='TLDP006I002', line[tLDP008_FirstAppearIndex:]))
        for tLDP001, tLDP002 in zip(list_TLDP006I001, list_TLDP008I002):
            t_time = timeCaculation(tLDP001,tLDP002)
            OutputList.append('{0},{1},{2},{3}'.format('マスタ同期', str(tLDP001[1]), str(tLDP002[1]), str(t_time)))

    ###TLDP008I001 マスタ同期

    fileinfo = pd.DataFrame({'cl':OutputList})
    fileinfo = pd.DataFrame(fileinfo.cl.str.split(",",3).tolist(),columns=["type", "start", "end", "due"])
    fileinfo[["type", "start","end"]]=fileinfo[["type", "start","end"]].astype('str')
    fileinfo[["due"]]=fileinfo[["due"]].astype('float')

    print(fileinfo.dtypes)
    for jobCount in range(8):
